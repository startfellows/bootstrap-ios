...
public extension UIDevice {
    ...
    var type: Model? {
        ...
        let code = withUnsafePointer(to: &systemInfo.machine, { pointer in
        ...
        guard let code = code,
              let string = String(validatingUTF8: code),
              let model = map[string]
        else {
            return nil
        }
        
        if model == .simulator {
            if let simulatorCode = ProcessInfo().environment["SIMULATOR_MODEL_IDENTIFIER"],
               let simulatorCodeString = String(validatingUTF8: simulatorCode),
               let simulatorModel = map[simulatorCodeString]
            {
